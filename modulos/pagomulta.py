    ...
    try:
        ...
        for multa in multas_data:
            resultado.append({
                ...
                'fecha_multa': multa.get('fecha_multa')
            })
...
def mostrar_pago_multas():
    ...
    try:
        ...
        frecuencia = reglamento['frecuencia_reunion'] if reglamento and reglamento.get('frecuencia_reunion') else 'SEMANAL'  # Valor por defecto
        
        # Calcular fecha límite de pago basado en la frecuencia (informativo)
        ...
        st.warning(f"⏰ **Fecha límite de pago (estimada):** {fecha_limite_pago} (Frecuencia: {frecuencia})")

        # Obtener SOLO las multas registradas para este grupo que aún tienen saldo pendiente
        cursor.execute("""
            SELECT 
                mxm.ID_Miembro,
                mxm.ID_Multa,
        ...
        resumen_pagos = cursor.fetchone() or {'total_multas_pagadas': 0, 'total_monto_pagado': 0}
        ...
        if multas_pendientes:
            ...
            multas_vencidas = [m for m in multas_pendientes if m['dias_transcurridos'] is not None and m['dias_transcurridos'] > 7]
            ...
            for multa in multas_pendientes:
                ...
                dias_transcurridos = multa.get('dias_transcurridos', 0)
                
                # Determinar si está vencida (regla: > 7 días desde la multa)
                esta_vencida = dias_transcurridos is not None and dias_transcurridos > 7
                ...
                with st.container():
                    ...
                    with col3:
                        ...
                        if esta_vencida:
                            ...
                            st.info("Se acumulará multa para la siguiente reunión si no se paga.")
                            ...
                            with col_btn1:
                                if st.button("💳 Pagar", key=f"btn_{multa['ID_Miembro']}_{multa['ID_Multa']}"):
                                    try:
                                        ...
                                        # Registrar en histórico de pagos
                                        try:
                                            cursor.execute("""
                                            ...
                                            """, (
                                                ...
                                                datetime.now().date(),
                                                id_reunion,
                                                None  # puede calcularse y guardarse si hace falta
                                            ))
                                        except Exception:
                                            # Si la tabla PagoMulta no acepta algun campo, continuar sin interrumpir
                                            pass
                    ...
                    try:
                        ...
                        for multa in multas_pendientes:
                            ...
                            if saldo_pendiente > 0:
                                ...
                                # Intento de registro en PagoMulta (no crítico)
                                try:
                                    cursor.execute("""
                                        INSERT INTO PagoMulta (ID_Miembro, ID_Multa, monto_pagado, fecha_pago, ID_Reunion_pago)
                                        VALUES (%s, %s, %s, %s, %s)
                                    """, (multa['ID_Miembro'], multa['ID_Multa'], Decimal(str(saldo_pendiente)), datetime.now().date(), id_reunion))
                                except Exception:
                                    pass
