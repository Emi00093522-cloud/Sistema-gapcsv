...
def mostrar_multas():
    ...
    try:
        ...
        # Cargar TODOS los miembros del grupo y su estado de asistencia (sin columna justificado)
        cursor.execute("""
            SELECT 
                m.ID_Miembro, 
                m.nombre,
        ...
        # Separar miembros por estado de asistencia (sin justificados)
                    ...
                    if multa['ID_Estado_multa'] == 1:  # Pendiente
                        ...
                        if st.button("↩️ Marcar como Pendiente", key=f"revertir_{multa['ID_Multa']}"):
                            try:
                                ...
                                """, (multa['ID_Multa'],))
                                con.commit()
                                st.warning(f"↩️ Multa de {multa['nombre_miembro']} marcada como pendiente.")
                                st.rerun()
                            except Exception as e:
                                con.rollback()
                                st.error(f"❌ Error al actualizar: {e}")
